...
public:
  ...
  void invert()  {
    ...
    for ( int i = 0; i < actualsize; i++ )   // invert U
      for ( int j = i; j < actualsize; j++ )  {
        if ( i == j ) continue;
        D sum = Vector2(0.0, 0.0);
        for ( int k = i; k < j; k++ )
            sum += data[k*maxsize+j]*( (i==k) ? Vector2(1.0, 0) : data[i*maxsize+k] );
        data[i*maxsize+j] = Vector2(0.0, 0.0) - sum;
        }
    for ( int i = 0; i < actualsize; i++ )   // final inversion
      for ( int j = 0; j < actualsize; j++ )  {
        D sum = Vector2(0.0, 0.0);
        for ( int k = ((i>j)?i:j); k < actualsize; k++ )  
            sum += ((j==k)?Vector2(1.0, 0):data[j*maxsize+k])*data[k*maxsize+i];
        data[j*maxsize+i] = sum;
        }
    };
};
