...
VoronoiDiagramGenerator::~VoronoiDiagramGenerator()
{
    LOG<<"In ~VoronoiDiagramGenerator()"<<'\n';
	reset();
...
{
    LOG<<"In VoronoiDiagramGenerator::reset()"<<'\n';
	cleanup();
	cleanupEdges();
    ...
    LOG<<"At end of VoronoiDiagramGenerator::reset()"<<'\n';
...
struct Site * VoronoiDiagramGenerator::intersect(struct Halfedge *el1, struct Halfedge *el2, struct PointVDG *)
{
	struct	Edge *e1,*e2, *e;
	struct  Halfedge *el;
	float d, xint, yint;
...
void VoronoiDiagramGenerator::circle(float , float , float ){}
void VoronoiDiagramGenerator::range(float , float , float , float ){}
...
void VoronoiDiagramGenerator::out_ep(struct Edge *)
{
	//if(!triangulate & plot) 
	//clip_line(e);
	//if(!triangulate & !plot)
...
void VoronoiDiagramGenerator::out_vertex(struct Site *)
{
	if(!triangulate & !plot &!debug)
	{
		//printf ("v %f %f\n", v->coord.x, v->coord.y);
...
void VoronoiDiagramGenerator::out_site(struct Site *)
{/*
	if(!triangulate & plot & !debug)
...
void VoronoiDiagramGenerator::out_triple(struct Site *, struct Site *,struct Site * )
{
	//if(triangulate & !plot &!debug)
	//{
...
bool VoronoiDiagramGenerator::voronoi(bool genVertexInfo)
{
	...
	if(genVertexInfo)
	{
        ...
        LOG<<"After counting the size of the final vertices = "<<sizeOfFinalVertices<<'\n';
        ...
        LOG<<"Just before generateVertexLinks()"<<'\n';

		generateVertexLinks();
        LOG<<"Just after generateVertexLinks()"<<'\n';

	}
	
    LOG<<"After generateVertexLinks()"<<'\n';
	cleanup();
...
void VoronoiDiagramGenerator::insertVertexAddress(long vertexNum, struct Site* address)
{
	...
	while(vertexNum > sizeOfVertices - 1)
	{
        ...
        LOG<<"Grew vertices to size "<<sizeOfVertices<<" and vertices = "<<vertices<<'\n';
	}
...
void VoronoiDiagramGenerator::insertVertexLink(long vertexNum, long vertexLinkedTo)
{
	...
	{
        //LOG<<"Resizing the array to "<<sizeOfVertexLinks + 4000<<'\n';
        //LOG<<'\n';
		vertexLinks = (Point3*)realloc(vertexLinks,(sizeOfVertexLinks+4000)*sizeof(Point3));
        //if(vertexLinks == 0)LOG<<"Error - realloc failed, vertexLinks == 0"<<'\n';
		sizeOfVertexLinks += 4000;
...
void VoronoiDiagramGenerator::generateVertexLinks()
{
    ...
    LOG<<"sizeOfFinalVertexLinks = "<<sizeOfFinalVertexLinks<<",sizeOfVertexLinks = "<<sizeOfVertexLinks<<'\n';
    ...
    LOG<<"Created vertexLinks array of size "<<sizeOfFinalVertexLinks<<'\n';
    ...
    LOG<<"Before copying "<<sizeOfVertices<<" coordinates into finalVertexLinks"<<'\n';
	for(i = 0; i< sizeOfVertices; i++)
	{		
    //	LOG<<"copying "<<i<<" coords,vertices[i]="<<vertices[i]<<'\n';
		if(vertices[i] == 0)
			break;
    ...
    LOG<<"After copying "<<i+1<<" coordinates into finalVertexLinks"<<'\n';
    ...
    LOG<<"About to process the "<<count1counter<<" count 1 (leaf) vertices"<<'\n';
    ...
    LOG<<"About to process the "<<count3counter<<" count 3 (non-leaf) vertices"<<'\n';
    ...
    //	LOG<<"after count3links, finalVertexLinks[count3vertices[i]].count = "<<finalVertexLinks[count3vertices[i]].count<<'\n';
    ...
    //	LOG<<"after finalVertexLinks[count3vertices[i]]"<<'\n';
    ...
    LOG<<"Finished processing the count 3 vertices"<<'\n';
}
