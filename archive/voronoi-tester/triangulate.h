...
namespace Delaunay
{
	...
	template<typename PointType>
	SimpleMesh<PointType> triangulate( std::vector<PointType> input_points )
	{
		...
		for (int i = 0; i < nv; i++)
		{
			p[i].x = input_points[i].x();
			p[i].y = input_points[i].y();
			p[i].z = 0;
		}
		...
		SimpleMesh<PointType> mesh;
		
		for (int i = 0; i < nv; i++)
		{
			std::vector < double > verts;
			verts.push_back(p[i].x);
			verts.push_back(p[i].y);
		...
		for (int i = 0; i < ntri; i++)
		{
			...
			verts.push_back(v[i].p1);
			verts.push_back(v[i].p2);
			verts.push_back(v[i].p3);
			mesh.faces.push_back(verts);
		}
